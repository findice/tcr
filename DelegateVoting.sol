...
contract DelegateVoting {
    ...
    event _VotingRightsTransfer(uint numTokens, address delegatedvote,address delegateevote);
    // ============
    // DATA STRUCTURES:
    // ============
    ...
    mapping(address => uint) public voteTokenSent;//record the token Represented vote sent.
    ...
    */
    ...
    /**
    @notice delegaterightsent and delegaterightchange record the vote right transfer.
    @param _pollID Integer identifier associated with target poll
    @param _to  The delegateevote address.
    @param _numTokens The number of tokens to be committed towards the target poll
   
    */

    function delegaterightsent(uint _numTokens,address _to, uint _pollID)public{
        ...
        voteTokenSent[msg.sender]=voteTokenSent[msg.sender].add(_numTokens);
        voteTokenBalance[_to] = voteTokenBalance[_to] .add(_numTokens);
        emit _VotingRightsTransfer(_numTokens, msg.sender,_to);

    }

    function delegaterightchange(uint _numTokens,address _to, uint _pollID)public{
        require(commitPeriodActive(_pollID));
        require(voteTokenSent[msg.sender]>=_numTokens);
        voteTokenBalance[msg.sender] = voteTokenBalance[msg.sender].add(_numTokens);
        voteTokenSent[msg.sender] = voteTokenSent[msg.sender].sub(_numTokens);
        voteTokenBalance[_to] = voteTokenBalance[_to] .sub(_numTokens);
        emit _VotingRightsTransfer(_numTokens, msg.sender,_to);
