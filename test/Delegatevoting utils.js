...
const utils = {
  ...
  delegaterightsent(pollID, voteOption, tokensArg, salt, , delegatedvoter,delegateevoter, voting) => {
	const hash = utils.getVoteSaltHash(voteOption, salt);
    await utils.as(delegatedvoter, voting.requestVotingRights, tokensArg);
	await utils.as(delegatedvoter, voting.delegaterightsent, uint _numTokens,address _to, uint _pollID);

	
  }
  votingRightget(pollID,voter)=>{
	  await utils.as(voter,voting.getvoteright,voter,pollID);
	 // const num = await utils.as(delegatedvoter, voting.delegaterightsent, tokensArg,delegateevoter, uint _pollID);

	  return num.logs[0].args.tokensArg;
